...
def perLocation(lo, url):
    ...
    for df in dfs:
        ...
        UrlBufferLock.acquire()
        urlQueue.put((url, lo + location[0]))
        #item += pageData(lo + '-' + location[0], url)
        UrlBufferLock.release()
...
def threadPageData():
    ...
    while(True):
        ...
        url = urlQueue.get()
        UrlBufferLock.release()
        item = pageData(url[1], url[0])
        DatBufferLock.acquire()
        dataBuff += item
        DatBufferLock.release()
...
def writeThread():
    ...
    while(True):
        ...
        test.to_csv('E:\\31\\test4.csv', mode='a',
...
def MainThread(url):
    t2 = Thread(name='anlisyURL', target=EveryContryURL, args=(url,))
    t2.start()
    time.sleep(2)
    for i in range(5):
        t = Thread(target=threadPageData)
        threads.append(t)
        t.start()

    t = Thread(name='WriteThread', target=writeThread)
    time.sleep(10)
    t.start()
...
url = 'https://zh.weatherspark.com'
t = Thread(target=MainThread, args=(url, ))
t.start()
